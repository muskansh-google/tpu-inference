...
from tpu_inference.layers.vllm.mla_attention import VllmTPUMultiHeadLatentAttentionWrapper
from tpu_inference.layers.vllm.process_weights.cleanup_sharding import shard_model_to_tpu
...
class VllmModelWrapper:
  ...
  def load_weights(self):
    ...
    if (
        hasattr(self.vllm_config.device_config, "slice")
        and self.vllm_config.device_config.slice is not None
    ):
    ...
    assert (
        self.vllm_config.model_config.dtype in TORCH_DTYPE_TO_JAX
    ), "The model_config.dtype must be a PyTorch dtype."
    ...
    use_random_weights = vllm_config_for_load.load_config.load_format == "dummy"
    ...
    load_context = (
        patch("torch._sync", return_value=None)
        if use_random_weights
        else nullcontext()
    )
    ...
    static_forward_context = (
        vllm_config_for_load.compilation_config.static_forward_context
    )
    ...
    logger.info(
        "Total time to load model weights from storage to TPU:"
    ...
    def step_fun(
      ...
      with (
        ...
        replace_lora_metadata(
            self.model, original_lora_metadata, self.vllm_config.lora_config
        )
  ...
  def jit_compute_logits_func(self):
    ...
    @jax.jit(
        out_shardings=(
            NamedSharding(
                self.mesh,
                PartitionSpec(
                    ShardingAxisName.MLP_DATA, ShardingAxisName.MLP_TENSOR
                ),
            )
        )
    )
    def compute_logits_func(
      ...
      with (
          torchax.default_env(),
          set_vllm_model_wrapper_context(kv_caches=None, mesh=self.mesh),
      ):
        original_lora_metadata = replace_lora_metadata(
            self.model, lora_metadata, self.vllm_config.lora_config
        )
  ...
  def jit_embed_input_ids_func(self):
    ...
    def embed_input_ids_func(
      ...
      with torchax.default_env():
        output = self.model.embed_input_ids(
            torch_view(input_ids),
            torch_view(positions),
            torch_view(input_embeds) if input_embeds is not None else None,
        )
  ...
  def jit_embed_multimodal_fn(self):
    ...
    @functools.partial(
        jax.jit,
        static_argnames=("kwarg_keys", "image_grid_thw", "video_grid_thw"),
    )
    def embed_multimodal_func(
        params_and_buffers: Any,
        image_grid_thw: Optional[Tuple[Tuple[int, int, int], ...]],
        video_grid_thw: Optional[Tuple[Tuple[int, int, int], ...]],
        kwarg_keys: Sequence[str],
        kwarg_values: Sequence[jax.Array],
    ):
      # reconstruct kwargs
      kwargs = {}
      for key, value in zip(kwarg_keys, kwarg_values):
        kwargs[key] = torch_view(value)

      # Reconstruct grid_thw from tuple of tuples
      if image_grid_thw:
        kwargs["image_grid_thw"] = torch.tensor(
            image_grid_thw, device=self.vllm_config.device
        )
      if video_grid_thw:
        kwargs["video_grid_thw"] = torch.tensor(
            video_grid_thw, device=self.vllm_config.device
        )

      with torchax.default_env():
        output = self.model.embed_multimodal(**kwargs)
      if isinstance(output, (list, tuple)):
        return [jax_view(x) for x in output]
      return jax_view(output)

    def embed_multimodal_adapter(
        params_and_buffers: Any,
        image_grid_thw: Optional[Tuple[Tuple[int, int, int], ...]] = (),
        video_grid_thw: Optional[Tuple[Tuple[int, int, int], ...]] = (),
        **kwargs,
      ...
      return embed_multimodal_func(
          params_and_buffers, image_grid_thw, video_grid_thw, keys, values
      )

    return embed_multimodal_adapter
...
def load_lora_model(
    model: torch.nn.Module, vllm_config: VllmConfig, device: str
) -> torch.nn.Module:
