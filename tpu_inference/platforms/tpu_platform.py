...
class TpuPlatform(Platform):
  ...
  def get_attn_backend_cls(
    ...
    if use_mla:
      ...
      logger.info(
          "Cannot use %s backend on TPU. Setting to FLASH_ATTN.",
          selected_backend,
      )
  ...
  def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
    ...
    if vllm_envs.VLLM_TPU_USING_PATHWAYS:
      assert not vllm_envs.VLLM_ENABLE_V1_MULTIPROCESSING, (
          "VLLM_ENABLE_V1_MULTIPROCESSING must be 0 when using"
          " Pathways(JAX_PLATFORMS=proxy)"
      )
    ...
    parallel_config.worker_cls = "tpu_inference.worker.tpu_worker.TPUWorker"

    multihost_backend = envs.TPU_MULTIHOST_BACKEND
    if not multihost_backend:  # Single host
      if parallel_config.pipeline_parallel_size == 1:
        logger.info(
            "Force using UniProcExecutor for JAX on "
            "single host without pipeline parallelism."
        )
        parallel_config.distributed_executor_backend = "uni"
      else:
        logger.info(
            "Force using MultiprocExecutor for JAX on "
            "single host with pipeline parallelism."
        )
        from tpu_inference.executors.multiproc_executor import MultiprocExecutor
      ...
      logger.info("Force using RayDistributedExecutor for JAX on multihost.")
    else:
      logger.warning(
