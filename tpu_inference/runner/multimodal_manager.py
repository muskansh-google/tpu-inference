...
class MultiModalManager:
  ...
  def calc_mrope_positions(self, scheduler_output: "VllmSchedulerOutput"):
    ...
    for index, req_id in enumerate(self.runner.input_batch.req_ids):
      ...
      num_computed_tokens = self.runner.input_batch.num_computed_tokens_cpu[
          index
      ]
  ...
  def execute_mm_encoder(self, scheduler_output: "VllmSchedulerOutput"):
    ...
    # encoder outputs.
    encoder_outputs = []
    for _, num_items, mm_kwargs_group in group_mm_kwargs_by_modality(mm_kwargs):
      ...
      for key, value in batched_mm_inputs.items():
        if isinstance(value, torch.Tensor):
          ...
          if value.dtype == torch.bfloat16:
            batched_mm_inputs[key] = (
                value.to(torch.float32).numpy().astype(jnp.bfloat16)
            )
          else:
      ...
      curr_group_outputs = self.runner.embed_multimodal_fn(
          self.runner.state, image_grid_thw, video_grid_thw, **batched_mm_inputs
      )
